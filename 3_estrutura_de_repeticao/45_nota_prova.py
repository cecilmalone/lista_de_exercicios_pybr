...
while True:
    for n in range(10):
        resposta = input("Informe a resposta da Questão nº {}: ".format(n+1))
        respostas. append({'codigo': aluno, 'questao': n+1,
                           'resposta': resposta})
    aluno += 1
    continuar = input("Informar nota de um novo aluno? ")
    if continuar.lower() not in ('s', 'sim'):
        break

print("Total de alunos com notas no sistemas: {}.".format(aluno))
